...
### Session Hygiene
- Use `/cost` to inspect session token spend (API-billed usage); if youâ€™re on a subscription, use `/stats` instead.
...
```
artifacts/
â”œâ”€â”€ PRD.md                      # Authoritative requirements (frozen)
â”œâ”€â”€ PROJECT_FACT.md             # Constraints (budget, timezone, policies)
â””â”€â”€ modelling/
    â”œâ”€â”€ source_registry.yaml
    â”œâ”€â”€ data_model.md
    â”œâ”€â”€ pipeline.md
    â”œâ”€â”€ citation_contract.md
    â”œâ”€â”€ alert_scoring.md
    â””â”€â”€ backlog.json
...
Follow: **Explore â†?Plan â†?Execute â†?Verify â†?Log**

1) **Explore**
- Read only the artifact(s) needed for this step.

2) **Plan**
- State exactly which files will change (ideally 1â€?).
...
- If verification fails, fix the root cause (donâ€™t suppress).
...
```
fetch â†?extract â†?normalize â†?chunk â†?index â†?retrieve â†?synthesize â†?validate citations â†?deliver
```
...
A. `artifacts/modelling/source_registry.yaml` âœ? 
...
D. `artifacts/modelling/citation_contract.md` âœ? 
...
- Cite stored evidence, not â€œmemoryâ€?
...
- Avoid multi-agent â€œteamsâ€?unless necessary.
...
- [ ] All deliverables (Aâ€“F) exist and validate
